#!/usr/bin/env python3
"""
JetBot Landmark CSV Logger  (real hardware version)
----------------------------------------------------
Identical to landmark_csv_logger.py but uses:

  /odom  (nav_msgs/Odometry)  instead of /atlas/odom_ground_truth

If no odometry is available, position columns are written as 0.0 so that
detection records are not lost — only coordinates will be missing.

CSV columns:
  index, item_scanned, time_elapsed_s, pos_x, pos_y, pos_z, yaw_deg

Run standalone:
  python3 jetbot_landmark_csv_logger.py

Or with a custom odometry topic:
  python3 jetbot_landmark_csv_logger.py \
      --ros-args -p odom_topic:=/my_robot/odom
"""

import csv
import math
import os
import time
from datetime import datetime

import rclpy
from rclpy.node import Node
from rclpy.qos import QoSProfile, ReliabilityPolicy, HistoryPolicy
from nav_msgs.msg import Odometry
from yolo_msgs.msg import DetectionArray

# ── CONFIG ────────────────────────────────────────────────────────────────────
LOG_DIR  = os.path.expanduser('~/ros2_ws/src/NTU_COMP30271_CW_RobotSim/landmark_logs')
MIN_CONF = 0.5      # minimum YOLO confidence to log
MIN_AREA = 1500     # minimum bounding-box area (px²)
# ─────────────────────────────────────────────────────────────────────────────


def quat_to_yaw_deg(qx: float, qy: float, qz: float, qw: float) -> float:
    """Convert a quaternion to yaw angle in degrees."""
    yaw_rad = math.atan2(
        2.0 * (qw * qz + qx * qy),
        1.0 - 2.0 * (qy * qy + qz * qz),
    )
    return math.degrees(yaw_rad)


class JetBotLandmarkCSVLogger(Node):

    def __init__(self):
        super().__init__('jetbot_landmark_csv_logger')

        # ── Parameter: odometry topic name ───────────────────────────────────
        self.declare_parameter('odom_topic', '/odom')
        odom_topic = self.get_parameter('odom_topic').value

        # ── CSV setup ─────────────────────────────────────────────────────────
        os.makedirs(LOG_DIR, exist_ok=True)
        ts            = datetime.now().strftime('%Y-%m-%d-%H-%M')
        self.csv_path = os.path.join(LOG_DIR, f'landmarks_{ts}.csv')

        with open(self.csv_path, 'w', newline='') as f:
            csv.writer(f).writerow([
                'index', 'item_scanned', 'time_elapsed_s',
                'pos_x', 'pos_y', 'pos_z', 'yaw_deg',
            ])

        # ── Robot pose (relative to start) ────────────────────────────────────
        self.rel_x   = 0.0
        self.rel_y   = 0.0
        self.rel_z   = 0.0
        self.yaw_deg = 0.0

        self.start_x = None  # type: float
        self.start_y = None  # type: float
        self.start_z = None  # type: float

        self._odom_received = False

        # ── Timing / bookkeeping ──────────────────────────────────────────────
        self.start_time = time.monotonic()
        self.index      = 1
        self.seen       = set()
        self.rows       = []

        # ── Subscriptions ─────────────────────────────────────────────────────
        best_effort_qos = QoSProfile(
            reliability=ReliabilityPolicy.BEST_EFFORT,
            history=HistoryPolicy.KEEP_LAST,
            depth=5,
        )

        self.create_subscription(
            Odometry, odom_topic, self._odom_cb, best_effort_qos,
        )
        self.create_subscription(
            DetectionArray, '/detections', self._detection_cb, 10,
        )

        self.get_logger().info(f'JetBot Landmark CSV Logger started')
        self.get_logger().info(f'Odometry topic : {odom_topic}')
        self.get_logger().info(f'Saving to      : {self.csv_path}')

        # Warn if no odom after 5 s
        self.create_timer(5.0, self._check_odom)

        print(f'\n[JetBotLandmarkCSVLogger] Output → {self.csv_path}')
        print(f'  Odometry topic : {odom_topic}')
        print(f'  Min confidence : {MIN_CONF}')
        print(f'  Min bbox area  : {MIN_AREA} px²')
        print('  Waiting for detections...  (Ctrl+C to stop)\n')

    # ── One-shot odom check ───────────────────────────────────────────────────

    def _check_odom(self) -> None:
        if not self._odom_received:
            self.get_logger().warn(
                'No odometry received after 5 s. '
                'Positions will be logged as 0.0. '
                'Check that your odometry node is running '
                'or set odom_topic parameter correctly.'
            )

    # ── Odometry callback ─────────────────────────────────────────────────────

    def _odom_cb(self, msg: Odometry) -> None:
        self._odom_received = True
        pos = msg.pose.pose.position
        ori = msg.pose.pose.orientation

        if self.start_x is None:
            self.start_x = pos.x
            self.start_y = pos.y
            self.start_z = pos.z
            self.get_logger().info(
                f'Start position latched: '
                f'({self.start_x:.3f}, {self.start_y:.3f}, {self.start_z:.3f})'
            )

        self.rel_x   = pos.x - self.start_x
        self.rel_y   = pos.y - self.start_y
        self.rel_z   = pos.z - self.start_z
        self.yaw_deg = quat_to_yaw_deg(ori.x, ori.y, ori.z, ori.w)

    # ── Detection callback ────────────────────────────────────────────────────

    def _detection_cb(self, msg: DetectionArray) -> None:
        for d in msg.detections:
            if d.score < MIN_CONF:
                continue
            area = d.bbox.size.x * d.bbox.size.y
            if area < MIN_AREA:
                continue

            name = d.class_name
            if name in self.seen:
                continue
            self.seen.add(name)

            elapsed = round(time.monotonic() - self.start_time, 2)
            row = [
                self.index,
                name,
                elapsed,
                round(self.rel_x, 3),
                round(self.rel_y, 3),
                round(self.rel_z, 3),
                round(self.yaw_deg, 2),
            ]
            self.rows.append(row)

            with open(self.csv_path, 'a', newline='') as f:
                csv.writer(f).writerow(row)

            self.get_logger().info(
                f'[{self.index}] {name:<20}  conf={d.score:.2f}  '
                f't={elapsed:.1f}s  '
                f'pos=({self.rel_x:.2f},{self.rel_y:.2f},{self.rel_z:.2f})  '
                f'yaw={self.yaw_deg:.1f}°'
            )
            print(
                f'  [{self.index:>3}]  {name:<22}  '
                f't={elapsed:>7.1f}s  '
                f'pos=({self.rel_x:>6.2f}, {self.rel_y:>6.2f}, '
                f'{self.rel_z:>6.2f})  '
                f'yaw={self.yaw_deg:>7.1f}°'
            )
            self.index += 1

    # ── Summary ───────────────────────────────────────────────────────────────

    def print_summary(self) -> None:
        total = time.monotonic() - self.start_time
        print(f'\n{"=" * 66}')
        print(f'  LANDMARK SESSION SUMMARY')
        print(f'  Total elapsed   : {total:.1f} s')
        print(f'  Landmarks logged: {len(self.rows)}')
        if self.rows:
            hdr = (
                f'  {"#":<5}  {"Item":<22}  {"t(s)":<9}  '
                f'{"X":>7}  {"Y":>7}  {"Z":>7}  {"Yaw°":>8}'
            )
            print(f'\n{hdr}')
            print(f'  {"-" * 63}')
            for r in self.rows:
                print(
                    f'  {r[0]:<5}  {r[1]:<22}  {r[2]:<9}  '
                    f'{r[3]:>7}  {r[4]:>7}  {r[5]:>7}  {r[6]:>8}'
                )
        print(f'\n  CSV saved to: {self.csv_path}')
        print(f'{"=" * 66}\n')


def main(args=None) -> None:
    rclpy.init(args=args)
    node = JetBotLandmarkCSVLogger()
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        node.print_summary()
    finally:
        node.destroy_node()
        rclpy.shutdown()


if __name__ == '__main__':
    main()
